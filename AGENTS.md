...
- Estado actual de la DB remota (specs 07–13; réplica 1:1 en `supabase/migrations/`):
...
- La app ya lee/escribe la DB en todos los módulos: login real, `/kids`, `/kids/[id]`, invitaciones con Resend, activación de cuenta y **feed** (`/` lee `posts`/`post_children` ordenados por `published_at desc` y publica vía server action `createPost`). Ya no queda ningún módulo hardcodeado. Próximo candidato: SPEC 14 fotos (`post_photos` + bucket privado, en borrador).
...
- Next.js 16 + App Router. **No hay `src/`**: el código vive en `app/` en la raíz. Rutas:
  - `app/page.tsx` (feed, `/`) → **server component** que resuelve staff/daycare/sala (`getClaims()` → `users` → `daycares`/`rooms`), niños de la sala del staff y `posts` + `post_children` + autor ordenados por `published_at desc`; delega la UI en `FeedClient` (client: grupos por día, modal, `submitting`/`error`, `router.refresh()`). Sin `seedPosts` ni `Counter` (borrado).
...
- Tipos y utilidades de dominio en `app/lib/`: `dates.ts` (`formatDayDividerLabel`, `getLocalDayKey` para el feed), `kids-utils.ts` (`ChildRow`, `childRowToKid`, `buildParentRows`, alergias), `posts-utils.ts` (`PostTypeValue`, `PostRow`, `postRowToCard`, `groupPostsByDay`, `buildRecipient(childFirstNames, roomName)`). `app/data/kids.ts` conserva solo los tipos (`Kid`, `KidBadge`, `LinkedParent`); `app/data/rooms.ts` quedó sin uso (las salas vienen de la DB).
- Modales: `AddKidModal` (`/kids`), `LinkParentModal` (`/kids/[id]`) y `CreatePostModal` (feed) ya **persisten en la DB**. `CreatePostModal` recibe los niños por props (sala del staff), ofrece 3 pills excluyentes (toda la sala / anuncio general / niños concretos), publica contra `createPost` y ya no tiene bloque FOTOS. Todos validan en español y reusan estados `submitting | sent | error`.
- `PostCard`: `id` en props, autor real, hora desde `published_at`, sin enlaces a `/detalle-publicacion` ni `/crear-publicacion`; likes/comentarios siguen estáticos en 0 y el campo `image` queda como enganche de SPEC 14. `FeedHeader` muestra daycare/staff/sala/cantidad de niños reales. Estado vacío cuando no hay posts.
